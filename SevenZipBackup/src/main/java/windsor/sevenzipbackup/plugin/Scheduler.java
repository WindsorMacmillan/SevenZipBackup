...
public class Scheduler {
    ...
    public static void startBackupThread() {
        ...
        if (config.backupScheduling.enabled) {
            ...
            for (BackupScheduleEntry entry : config.backupScheduling.schedule) {
                ...
                for (DayOfWeek day : entry.days) {
                    ZonedDateTime previousOccurrence = ZonedDateTime.now(timezone)
                            ...
                            .with(ChronoField.CLOCK_HOUR_OF_DAY, entry.time.get(ChronoField.CLOCK_HOUR_OF_DAY)).withMinute(entry.time.get(ChronoField.MINUTE_OF_HOUR)).withSecond(0);
            ...
            scheduleDriftTask = taskScheduler.runTaskTimer(SevenZipBackup.getInstance(), Scheduler::startBackupThread, driftInt, driftInt).getTaskId();
