package windsor.sevenzipbackup.uploaders.googledrive;
...
public class GoogleDriveUploader extends Uploader {
    ...
    public GoogleDriveUploader(UploadLogger logger) {
        ...
        try {
            ...
            logger.info(intl("shared-drive-error"));
            setErrorOccurred();
        } catch (Exception e) {
    ...
    private void retrieveNewAccessToken() throws Exception {
        ...
        service = new Drive.Builder(
            ...
            .setApplicationName("SevenZipBackup")
            .build();
    }
    ...
    public void test(java.io.File testFile) {
        try {
            ...
            body.setDescription("SevenZipBackup test file");
    ...
    public void uploadFile(java.io.File file, String type) {
        try {
            ...
            fileMetadata.setDescription("Uploaded by the SevenZipBackup Minecraft plugin");
            ...
            try {
                ...
                if (!Strings.isNullOrEmpty(sharedDriveId)) {
                    logger.info(intl("backup-method-shared-drive-prune-failed"));
                } else {
                    logger.info(intl("backup-method-prune-failed"));
                }
                throw e;
            }
        } catch (Exception exception) {
    ...
    public void setupSharedDrives(CommandSender initiator) {
        if (drives != null && !drives.isEmpty()) {
            logger.info(intl("google-pick-shared-drive"));
            logger.info(
    ...
    public void finalizeSharedDrives(CommandSender initiator, String input) {
        ...
        logger.info(intl("link-provider-failed"), "provider", getAuthProvider().getName());
    }
